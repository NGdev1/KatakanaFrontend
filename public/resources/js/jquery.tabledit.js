...
(function ($) {
    ...
    $.fn.Tabledit = function (options) {
        ...
        const $lastEditedRow = 'undefined';
        let $lastDeletedRow = 'undefined';
        let $lastRestoredRow = 'undefined';
            ...
            columns: {
                identifier: function () {
                    ...
                    $td.each(function () {
                        ...
                        const span = '<span class="tabledit-span tabledit-identifier">' + $(this).text() + '</span>';
                        const input = '<input class="tabledit-input tabledit-identifier" type="hidden" name="' + settings.columns.identifier[1] + '" value="' + $(this).text() + '" disabled>';
                ...
                editable: function () {
                    for (let i = 0; i < settings.columns.editable.length; i++) {
                        ...
                        $td.each(function () {
                            ...
                            // Create span element.
                            const span = '<span class="tabledit-span">' + text + '</span>';
                ...
                toolbar: function () {
                    if (settings.editButton || settings.deleteButton) {
                        ...
                        const toolbar = '<div class="tabledit-toolbar ' + settings.toolbarClass + '" style="text-align: left;">\n\
        ...
        const Mode = {
            ...
            edit: function (td) {
                ...
                // Get input element.
                const $input = $(td).find('.tabledit-input');
        ...
        const Edit = {
            reset: function (td) {
                $(td).each(function () {
                    // Get input element.
                    const $input = $(this).find('.tabledit-input');
                    // Get span text.
                    const text = $(this).find('.tabledit-span').text();
            ...
            submit: function (td) {
                ...
                const ajaxResult = ajax(settings.buttons.edit.action, settings.buttons.edit.method);
        ...
        var Delete = {
            ...
            submit: function (td) {
                ...
                // Send AJAX request to server.
                const ajaxResult = ajax(settings.buttons.delete.action, settings.buttons.delete.method);
            ...
            restore: function (td) {
                ...
                $(td).parent('tr').find('input.tabledit-identifier').attr('disabled', false);
                // Send AJAX request to server.
                const ajaxResult = ajax(settings.buttons.restore.action);
                // Disable identifier hidden input.
                $(td).parents('tr').find('input.tabledit-identifier').attr('disabled', true);
        ...
        function ajax(action, method) {
            ...
            const serialize = $table.find('.tabledit-input').serialize();

            const result = settings.onAjax(action, serialize);
        ...
        if (settings.editButton) {
            ...
            $table.on('click', 'button.tabledit-edit-button', function (event) {
                if (event.handled !== true) {
                    ...
                    // Get current state before reset to view mode.
                    const activated = $button.hasClass('active');
            ...
            $(document).on('click', function (event) {
                const $editMode = $table.find('.tabledit-edit-mode');
        ...
        $(document).on('keyup', function (event) {
            ...
            const $input = $table.find('.tabledit-input:visible');
            const $button = $table.find('.tabledit-confirm-button');
